...
type HookMap = { [index: string]: unknown };
...
export const createScopeUse = <S extends UseBoundStore<StoreApi<object>>>(_store: S) => {
  ...
  for (const key of Object.keys(store.getState())) {
    if (use[key]) {
      throw `exists same key [${key}] in ${store}`;
    }
...
export const createUse = <S extends UseBoundStore<StoreApi<object>>>(_store: S) => {
  ...
  for (const key of Object.keys(store.getState())) {
    ...
    if (obj[useKey]) {
      throw `exists same key [${useKey}] in ${store}`;
    }
...
type WithSet<S> = S extends { getState: () => infer T }
  ? S & {
      [K in FilterState<S> as PrefixSet<K>]: (value: T[K] | ((oldValue: T[K]) => T[K])) => void;
    }
  : never;

export const createSet = <S extends UseBoundStore<StoreApi<object>>>(_store: S) => {
  const store = _store as WithSet<S>;
  const obj = store as unknown as HookMap;
  for (const key of Object.keys(store.getState())) {
    const setKey = `set${capitalize(key)}`;
    if (obj[setKey]) {
      throw `exists same key [${setKey}] in ${store}`;
    }
...
export const useSelector = <S, K extends readonly (keyof S)[]>(store: UseBoundStore<StoreApi<S>>, keys: K) => {
  return store(useShallow(state => keys.map(key => state[key]))) as unknown as {
    [index in keyof K]: S[K[index]];
  };
};

export const injectAction = <S extends UseBoundStore<StoreApi<object>>, A extends object>(_store: S, actions: A) => {
  const store = _store as S & A;

  for (const key of Object.keys(actions)) {
    const map = store as unknown as HookMap;
    if (map[key]) {
      throw `The key ${key} already exists in store`;
    }
